(function(){
  'use strict';
  const STORAGE_KEY='versoRecto.v48.experience';
  const SESSION_KEY='versoRecto.v48.sessions';
  const MAX_POSITIONS=12000;
  let pending=[];

  function empty(){return {schema:'verso-recto-experience-v48',version:48,updatedAt:null,totalGames:0,totalMovesLearned:0,positions:{}};}
  function load(){
    try{const x=JSON.parse(localStorage.getItem(STORAGE_KEY)||'null');return x&&x.version===48&&x.positions?x:empty();}
  ...
  function beginGame(){pending=[]; try{sessionStorage.removeItem('versoRecto.v48.pending');}catch(e){}}
  function recordMove(data){
    ...
    // Sauvegarde temporaire : évite de perdre la partie si la page est rechargée avant l'écran final.
    try{sessionStorage.setItem('versoRecto.v48.pending',JSON.stringify(pending));}catch(e){}
  }
  ...
  function finishGame(result){
    if(!pending.length){
      try{pending=JSON.parse(sessionStorage.getItem('versoRecto.v48.pending')||'[]')||[];}catch(e){pending=[];}
    }
    ...
    save(db); pending=[];
    try{sessionStorage.removeItem('versoRecto.v48.pending');}catch(e){}
    return true;
  }
  ...
  function exportData(){const db=load();const blob=new Blob([JSON.stringify(db,null,2)],{type:'application/json'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='verso-recto-v48-experience-'+new Date().toISOString().slice(0,10)+'.json';document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove();},0);}
  function importData(file){return file.text().then(t=>{const x=JSON.parse(t);if(!x||x.version!==48||!x.positions)throw new Error('Fichier V48 invalide');save(x);return stats();});}
  function clear(){localStorage.removeItem(STORAGE_KEY);pending=[];try{sessionStorage.removeItem('versoRecto.v48.pending');}catch(e){}}
  ...
  window.VR48Memory={beginGame,recordMove,finishGame,bestMove,stats,exportData,importData,clear,positionKey,markSession};
})();

window.VR44Memory = window.VR48Memory; // compatibilité du moteur historique
window.VR45Memory = window.VR48Memory; // compatibilité de la page Expérience issue de la V45
