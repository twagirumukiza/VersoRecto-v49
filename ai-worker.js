...
function chooseAIMove(player) {
  ...
  for (let depth = 1; depth <= profile.depth; depth++) {
    ...
    for (const move of roots) {
      ...
      const tieBreak = staticMoveScore(player, move.piece, move.to, player) + centralConnectionScore(player, move);
      const bestTieBreak = staticMoveScore(player, depthBest.piece, depthBest.to, player) + centralConnectionScore(player, depthBest);
